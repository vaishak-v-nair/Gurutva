![Gurutva — gravity, with error bars](docs/social-preview.png)
...
Point it at a four-column CSV (`x,y,z,gz` — position, elevation, reading), answer three questions about your site, and it writes an HTML report next to your data.

It says one of three things, and it is allowed to say the first one:
...
| path | what it is |
|---|---|
| `src/product/` | the product layer: gates, verdict, prior, report, the window, the CLI |
| `src/` | the engine: prism and magnetic forward operators, posterior, meshes |
| `src/gurutva_core/` | the gates and the weak-lensing operator (same maths, 10²² × the distance) |
| `tests/` | 172 tests, including one for every bug ever introduced |
| `examples/` | runnable demos: geothermal, carbon storage, dark matter, plus sample CSVs |
| `notebooks/` | the numbered scripts that produced every figure, in order |
| `web/` | the landing page and its build, the bundler, the installer builder, the release gate |
| `figures/` | every figure in this README, regenerated by the notebooks |
| `docs/` | the published page, the pitch, the design and phase-2 notes |

## Three surfaces, one tested code path

```bash
# the window
Gurutva.exe                       # or: py -3 -m src.product.app

# the command line
py -3 -m src.product.cli report --survey examples/demo_survey.csv \
    --noise 0.05 --prior-sd 0.10 --corr-len 400 --out verdict.html

# the library
from src.product import verdict
```

The window calls `cli.main` and captures its printing; there is deliberately no second, friendlier implementation that could drift from the tested one, and [a test asserts that by reading the source](tests/test_app.py).
