...
- [x] **S2.5 (2026-08-03) — the retracted claim could not be rescued, and we measured why.** The fix prescribed by the retraction (teach the simulator within-unit heterogeneity so the real data becomes in-distribution) was built and **failed on physical grounds, which is itself the result**:
  - *Enrichment ladder* — shorter-wavelength basement relief explains steadily more of the real misfit at steadily lower amplitude: 12 RBF @2.0 km → 65.4% (needs |w|~4438 m), 24 @1.2 km → 83.1% (982 m), 40 @0.9 km → 88.4% (488 m). Residual 11.2 vs a noise floor of 8.0.
...
- [x] **Dual-horizon expansion (2026-08-04, DEC-008): `src/gurutva_core/` — one engine family, two mathematics.** Poisson's equation is scale-free, so subsurface voxels, planetary interfaces and lensing sheets share one inversion core. **Astrodynamics does not**: there the field is known and the *state* is uncertain, so it is ensemble propagation, not Poisson inversion. The core keeps both engines honestly separate; what they genuinely share is the potential model, the declared-prior discipline, and the **four-gate suite**, now domain-independent and reusable (`src/gurutva_core/gates.py`).
- [x] **First space measurement — and it contradicts the pitch it was built to demonstrate.** `examples/space_trajectory_uncertainty.py` sweeps five cislunar regimes (1,000-member ensembles vs linearised STM covariance, 1 km / 10 mm/s navigation uncertainty): benign transfer 0.95×, L1 lingering 0.95× / 1.04× at 20 days, close lunar flyby 0.93×, deep lunar encounter (673 km closest approach) **1.34×**. **Linearised covariance is adequate in 4 of 5 regimes**, so Gurutva does not claim standard tooling is broken. But the driver sweep then located the boundary precisely: hold the deep encounter and stretch the horizon or degrade navigation and linearisation collapses — **30 days → 13.3×, 20 days at 10 km nav → 9.9×, 20 days at 50 km nav → 50.9×** (there the true 1σ spread is 2,063 km against a linearised 1,364). **The product is that boundary**: short horizons with good navigation don't need probabilistic astrodynamics; deep encounters with long horizons or degraded navigation need it by an order of magnitude. Neither half of that sentence was assumed — both were measured. Pinned by `tests/test_gurutva_core.py` (which also verifies L1 equilibrium and Jacobi-constant conservation to 1e-9) — `figures/space_uncertainty_corridor.png`
- [x] **E2 — THE DARK-MATTER TWIN: P4 confirmed in code, all four gates pass (2026-08-05).** The premise this project was founded on — *"impossible and practical are the same math"* — was finally tested rather than asserted. `src/posterior.py`, written for Utah ore bodies, was run **completely unmodified** on the weak-lensing shear of a dark-matter halo (Kaiser-Squires operator, `src/gurutva_core/lensing.py`). It works, because both are the same linear potential-field problem: prisms→gravity and mass-sheet→shear share an operator signature, so the engine does not care what the mass is made of.
  - **Gate 1 caught a real mistake first**: the initial declared prior (σ=0.05) cannot produce a cluster halo, licensing failed at 100%, and the posterior peak came back biased low (0.35 vs 0.597 true) — textbook over-shrinkage. The prior was then re-declared **from halo physics** (σ=0.15, cluster-scale convergence), *never tuned until the gate turned green* — that S3 trap stays closed, and the too-narrow prior is kept as a permanent test.
  - **All four gates pass**: licensing 0.0 pct · MC calibration 1.5% median · stability exact · adequacy 0.021 vs 0.03 noise. **CLAIMABLE.**
  - **The result, in the language of both worlds at once**: halo peak recovered at **15.2σ**, detected above 2σ in 71 of 576 pixels, and — the object physicists actually publish — a **95% exclusion limit of κ < 0.060** wherever nothing is detected. That exclusion limit *is* the drill-decision posterior, computed by the same code. The romance and the revenue are one engine, demonstrated. `tests/test_lensing.py` · `figures/dark_matter_twin.png`
## The product

Two demos, one engine. Input: a survey and a model. Output: a one-page verdict — where the model is real, pass/fail on four gates, and the single number a decision turns on, with an interval.

- [x] **Demo 1 — the drilling verdict** (`examples/verdict_geothermal.py`, real Utah FORGE data). All four gates pass. 30% of cells informed, the model is invented below 1,322 m elevation, excess mass −346 Mt (95%: −484 to −207), and the standard workflow would have quoted that interval **2.6x too tight**. `figures/verdict_geothermal.html`
- [x] **Demo 3 — the exclusion verdict, and the hole it found in our own gates** (`examples/verdict_dark_matter.py`). The subsurface posterior engine, run unmodified on the weak-lensing shear of a dark-matter halo. All four gates pass: halo detected at **15.2σ**, 95% exclusion limit **κ < 0.037** where nothing is seen. **And then it caught itself** — the aperture-mass estimate missed a known truth by **11.6σ**, because the true peak is a 4.0σ excursion under our own declared prior and the posterior shrank it 23% toward zero. No gate saw it: licensing asks whether the *data* is plausible, never the truth; SBC draws its test truths *from* the prior and is blind by construction; two runs agreed on the same shrunken answer; a shrunken field still reproduces the shear. The mass estimate is therefore published as **diagnostic only and not claimed**; the exclusion limit survives because shrinkage makes an upper bound conservative. Same species of blind spot that created gate 4, found the same way. Open, deliberately: a fifth gate for coverage at a fixed realistic truth, or a prior class declared wide enough to contain the object being searched for. `figures/verdict_dark_matter.html`
- [x] **Demo 2 — the containment verdict** (`examples/verdict_carbon_storage.py`, Sleipner-class CO2 monitoring design; published survey parameters, simulated observations, stated plainly in the file and the report). All four gates pass. Inventory reconciles at 7.4 ± 7.1 Mt against 10 Mt injected; a 400 m accumulation 2 km outside the complex is detectable **only above 72% saturation**; a whole-area mass bound is computed and then **refused** because it is set by the prior, not the data. Measured, not asserted: a 2.7x quieter gravimeter buys 6% — this survey is limited by its geometry, not its noise. `figures/verdict_carbon_storage.html`
- [x] **The number the customer acts on is exact.** Mass in a region is a linear functional w'm, so its variance needs the off-diagonal posterior covariance; summing per-cell variances under-reports the interval, and over-confidence is the one direction this work may never be wrong in. Computed via the data-space identity and pinned against a dense covariance (`tests/test_product.py`).
- [x] **Two priors in common use were refused by gate 1, and both refusals are findings.** β from the discrepancy principle implies rock density varies by 0.021 g/cc — licensing fails at the 100th percentile. A flat independent-per-cell prior at a physical 0.25 g/cc is also refused (99th): uncorrelated draws are static, neighbours cancel, and the long-wavelength gravity a real basin makes never appears. `src/product/prior.py` builds the correlated prior the gate demanded, sampling through a SuperLU factor whose `U == D L^T` identity and permutation convention are **checked at construction** — a silent permutation bug would draw from the wrong distribution while every downstream gate stayed green.
- [x] **adequacy is now two-sided**, forced by measurement: S3 failed by missing the data; Utah with a physics prior fit to 0.54 mGal against a *measured* 0.76 floor. A model that explains the noise has absorbed it. Both are inadequacy.

- [ ] S3 revisited at ~744 coefficients (the measured requirement) · real survey data (DES/KiDS/Euclid) for the lensing adapter
