...
4. **Published to GitHub Pages and committed to the repo.** D4-C, revised on launch day 2026-08-02: the page is live at `https://vaishak-v-nair.github.io/Gurutva/`, source at `web/index.html`, rebuilt by `web/build_interactive.py`, which now also writes `docs/index.html` because Pages serves `main:/docs`. The original `web/build.py` was deleted the same day — two builders writing the same file, one of which still described the repository as private, is a landmine rather than a fallback. `gurutva.ai` remains unregistered; the link moves when it is.
